...
const OBJECT = [
    ...
    ['===', '===(other)', 'Case equality.'],
    ['=~', '=~(other)', 'Pattern match.'],
    ['hash', 'hash(other)', 'Appends this object\'s value to *hasher*, and returns the modified *hasher*.'],
    ['hash', 'hash', 'Generates an `UInt64` hash value for this object.'],
    ['to_s', 'to_s : String', 'Returns a string representation of this object.'],
    ['to_s', 'to_s(io : IO) : Nil', 'Appends a `String` representation of this object to the given `IO` object.'],
    ['inspect', 'inspect : String', 'Returns a `String` representation of this object suitable to be embedded inside other expressions, sometimes providing more information about this object.'],
    ['inspect', 'inspect(io : IO) : Nil', 'Appends a string representation of this object to the given `IO` object.'],
    ['pretty_print', 'pretty_print(pp : PrettyPrint) : Nil', 'Pretty prints `self` into the given printer.'],
    ['pretty_inspect', 'pretty_inspect(width = 79, newline = "\n", indent = 0) : String', 'Returns a pretty printed version of `self`.'],
    ['tap', 'tap', 'Yields `self` to the block, and then returns `self`.'],
    ['try', 'try', 'Yields `self`. `Nil` overrides this method and doesn\'t yield.'],
    ['in?', 'in?(collection : Object) : Bool', 'Returns `true` if `self` is included in the *collection* argument.'],
    ['in?', 'in?(*values : Object) : Bool', 'Returns `true` if `self` is included in the *collection* argument.'],
    ['not_nil!', 'not_nil!', 'Returns `self`.'],
    ['itself', 'itself', 'Returns `self`.'],
    ['dup', 'dup', 'Returns a shallow copy (“duplicate”) of this object.'],
    ['unsafe_as', 'unsafe_as(type : T.class) forall T', 'Unsafely reinterprets the bytes of an object as being of another `type`.'],
];
