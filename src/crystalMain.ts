...
const crystalConfiguration = {
	indentationRules: {
		increaseIndentPattern: /^\s*((begin|(abstract)\s+(class|struct)|class|struct|(private|protected)\s+def|def|fun|macro|else|elsif|ensure|for|if|module|rescue|unless|until|when|while)|([^#]*\sdo\b))\b[^\{;]*$/,
