...
class CrystalCompletionItemProvider implements CompletionItemProvider {
    ...
    async provideCompletionItems(
        ...
        const column = new Position(position.line, position.character > 2 ? position.character - 2 : 0);
        const dot = new Position(position.line, column.character + 1);
        const range = document.getWordRangeAtPosition(dot) || document.getWordRangeAtPosition(column);
        // TODO: check symbols
...
export function registerCompletion(selector: DocumentSelector, context: ExtensionContext): void {
    context.subscriptions.push(
        languages.registerCompletionItemProvider(selector, new CrystalCompletionItemProvider(), '.', '::')
    );
}
